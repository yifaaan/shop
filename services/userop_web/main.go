...
import (
	...
	"shop/services/userop_web/registry"
	"shop/services/userop_web/web"
...
func main() {
	...
	// 通过 Consul resolver 发现 userop_srv 并按 round_robin 负载均衡。
	useropConn, err := grpc.NewClient(
		fmt.Sprintf("consul://%s:%d/%s?healthy=true", cfg.Consul.Host, cfg.Consul.Port, cfg.UserOpSrv.Name),
		grpc.WithTransportCredentials(insecure.NewCredentials()),
		grpc.WithDefaultServiceConfig(`{"loadBalancingPolicy":"round_robin"}`),
	)
	if err != nil {
		log.Panic("failed to connect to userop service: ", err)
	}
	defer useropConn.Close()

	// 通过 Consul resolver 发现 goods_srv（收藏列表需补全商品名/图/价）。
	...
	srv := web.New(cfg, log, web.NewJWT(cfg), proto.NewUserOpClient(useropConn), proto.NewGoodsClient(goodsConn))
	httpSrv := &http.Server{
