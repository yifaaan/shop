...
func InitConfig() {
	...
	if getEnvInfo("SHOP_DEBUG") {
		debug = true
		viper.SetConfigFile("./order_srv/config-debug.yaml")
	} else {
		viper.SetConfigFile("./order_srv/config-pro.yaml")
	}
	...
	if content == "" || content == "null" {
		zap.S().Fatalf("nacos config empty: data_id=%s group=%s namespace=%s", global.NacosConfig.DataID, global.NacosConfig.Group, global.NacosConfig.Namespace)
	}
