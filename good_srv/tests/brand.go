...
func TestBannerList() {
	resp, err := brandClient.BannerList(context.Background(), &proto.Empty{})
	if err != nil {
		panic(err)
	}
	fmt.Println(resp.Total)
...
func TestCreateBanner() {
	rsp, err := brandClient.CreateBanner(context.Background(), &proto.BannerRequest{
...
func TestUpdateBanner(id int32) {
	_, err := brandClient.UpdateBanner(context.Background(), &proto.BannerRequest{
		Id:    id,
...
func TestDeleteBanner(id int32) {
	_, err := brandClient.DeleteBanner(context.Background(), &proto.BannerRequest{
		Id: id,
	})
	if err != nil {
		panic(err)
